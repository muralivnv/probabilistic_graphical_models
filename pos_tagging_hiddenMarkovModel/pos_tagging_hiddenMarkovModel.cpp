...
int main()
{
  ...
  const size_t n_tags = 12u;
  VEC<string, n_tags> tags;
  ...
  MAT<uint_t, n_tags, n_tags> tag_bigram_count;
  VEC<uint_t, n_tags> tag_unigram_count;

  unigram_count(labels, tags, train_indices, tag_unigram_count);
  bigram_count(labels, tags, train_indices, tag_bigram_count);

  VEC<uint_t, n_tags> starting_tag_count;
  VEC<uint_t, n_tags> ending_tag_count;
  ...
  MAT<float, n_tags, n_tags> state_transition_prob;
  ...
  HASH_MAP<std::string,       /* observation */
           VEC<float, n_tags> /* probability w.r.to each state */
           >  observation_prob;
